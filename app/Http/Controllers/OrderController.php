  ...
  <table width="100%">
    <tr>
        <td><strong>'.__("messages.from").':</strong> 
            <pre>'.$order->billing_first_name.'</pre>
            <pre>'.$order->billing_address.'</pre>
        </td>
  ...
  <table width="100%">
    ...
    ';
      foreach ($order_data as $od) {
          $html=$html.'<tr><td>'.$this->headreadMoreHelper($od->productdata->name,25).'</td><td>'.$res_curr[1].number_format((float)$od->price, 2, '.', '').'</td><td>'.$od->quantity.'</td><td>'.$res_curr[1].number_format((float)$od->total_amount, 2, '.', '').'</td></tr>';
      }
    ...
    <tfoot>
        <tr>
            <td colspan="2"></td>
            <td align="right">'.__("messages.subtotal").'</td>
            <td align="right">'.$res_curr[1].number_format((float)$order->subtotal, 2, '.', '').'</td>
        </tr>        
        <tr>
            <td colspan="2"></td>
            <td align="right">'.__("messages.shipping").'</td>
            <td align="right">'.$res_curr[1].number_format((float)$order->shipping_charge, 2, '.', '').'</td>
        </tr>
        <tr>
            <td colspan="2"></td>
            <td align="right">'.__("messages.taxes").'</td>
            <td align="right">'.$res_curr[1].number_format((float)$order->taxes_charge, 2, '.', '').'</td>
        </tr>
        <tr>
            <td colspan="2"></td>
            <td align="right" style="border-top: 1px solid lightgray;">Total</td>
            <td align="right" class="" style="border-top: 1px solid lightgray;">'.$res_curr[1].number_format((float)$order->total, 2, '.', '').'</td>
        </tr>
    </tfoot>
  </table>
';
  ...
  public function sendordermail(Request $request){
       ...
       $user->pdffile=asset('public/pdf/').'/'.$request->get("filename");
       Mail::send('email.view_order', ['user' => $user], function($message) use ($user){
   ...
   public function changeorderstatus($order_id,$staus_id){
            DB::beginTransaction();
              try {
                             ...
                             $android=$this->send_notification_android($setting->android_api_key,$order->user_id,$msg);
                             $ios=$this->send_notification_IOS($setting->iphone_api_key,$order->user_id,$msg); 
