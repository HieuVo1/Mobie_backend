...
class ApiController extends Controller
{
      ...
      if ($validator->fails()) {
         ...
         try {
            ...
            foreach ($product_ids as $k) {
               $data = CartData::where("product_id", $k)->first();
               $data->delete();
            }
   ...
   public function bestselling($user_id)
   {
      ...
      if (count($data) != 0) {
         foreach ($data as $k) {
            $dat = [];
            $getproduct = Product::where('status', '1')->where('is_deleted', '0')->where("id", $k->product_id)->first();
            if ($getproduct) {
   ...
   public function mainoffers()
   {
      ...
      foreach ($data as $k) {
         if ($k->offer->is_product == '1') {
            $k->offer->new_price = "";
            $k->offer->product_id = "";
         }
         if ($k->offer->is_product == '2') {
            $k->offer->fixed = "";
            $k->offer->category_id = "";
         }
         $best[] = $k->offer;
   ...
   public function viewproduct($id, $user_id)
   {
      ...
      if (!empty($product)) {
         ...
         foreach ($product->attributes as $ke) {
            $attributearr[] = $ke->attribute_id;
         }
         foreach (array_values(array_unique($attributearr)) as $att) {
            $attribueset = Attributes::with("setname")->find($att);
            $attribute_set[] = $attribueset->att_set_id;
         }
         foreach (array_values(array_unique($attribute_set)) as $attset) {
            $arr = array();
            $getname = AttributeSet::find($attset);
            $arr["set"] = $getname->name;
            foreach (array_values(array_unique($attributearr)) as $att) {
               $data = Attributes::where("att_set_id", $attset)->where("id", $att)->get();
               $attval = array();
               if (count($data) != 0) {
                  foreach ($data as $k) {
                     $attval[] = $k->id;
                  }
                  foreach (array_unique($attval) as $valarr) {
                     $findpro = ProductAttributes::where("product_id", $id)->where("attribute_id", $valarr)->get();
                     $getset = Attributes::find($valarr);
                     $newls = array();

                     $valu = array();
                     foreach ($findpro as $valattr) {
                        $arrbrk = explode(",", $valattr->values);
                        foreach ($arrbrk as $k) {
                           $getvalname = Attributevalues::find($k);
                           if (!empty($getvalname)) {
                              $valu[] = $getvalname->values;
                           }
                        }
                     }
                     $newls["name"] = $getset->name;
                     $newls["values"] = array_values(array_unique($valu));
                  }
                  $arr["attributename"][] = $newls;
               }
            }
            $main_array[] = $arr;
         }
         $product->attributes = $main_array;
         if ($product->additional_image != "") {
            $img = array();
            $images = explode(",", $product->additional_image);
            foreach ($images as $k) {
               $img[] = asset('upload/product/') . '/' . $k;
            }
         ...
         foreach ($product->review as $re) {
            $re->image = asset("upload/profile/") . $re->userdata['profile_pic'];

            // $re->userdata['profile_pic']=asset("upload/profile/").$re->userdata['profile_pic'];
         }
      }
      ...
      if ($request->input('login_type') == '2') {
         ...
         if ($checkuser) { //login
            ...
            if ($request->get("image") != "") {
               ...
               Image::make(file_get_contents(urldecode($request->get("image"))))->save($path);
   ...
   public function getwishlist(Request $request)
   {
      ...
      if ($validator->fails()) {
         ...
         if (count($data) != 0) {
            foreach ($data as $k) {
               $product = Product::where("id", $k->product_id)->select("id", "name", "basic_image", "selling_price")->first();
               $product->basic_image = asset('upload/product/') . '/' . $product->basic_image;
   ...
   {
      $response = array("success" => "0", "msg" => "Not Set");
      $rules = [
      ...
      if ($validator->fails()) {
         ...
         if (!$data) {
            $response = array("status" => 0, "msg" => "Coupon Not Found");
         } else {
            ...
            if (($date >= $start_date) && ($date <= $end_date)) {
               ...
               if ($data->usage_limit_per_coupon != "" && ($data->usage_limit_per_coupon < count($order))) {
                  ...
                  $response = array("status" => 0, "msg" => "Coupon Limit Over");
               } elseif ($data->usage_limit_per_customer != "" && ($data->usage_limit_per_customer <= count($orderuser))) {
                  $response = array("status" => 0, "msg" => "Your Coupon Limit Over");
               } elseif ($data->minmum_spend != "" && $data->minmum_spend >= $request->get("total")) {
                  $response = array("status" => 0, "msg" => "Not Vaild Coupon.total less than minimum amount of coupon");
               } elseif ($data->maximum_spend != "" && $data->maximum_spend <= $request->get("total")) {
                  $response = array("status" => 0, "msg" => "Not Valid Coupon. total greater than maximum amount of coupon");
               } else {
   ...
   public function searchproduct(Request $request)
   {
      ...
      if ($validator->fails()) {
         ...
         $product = Product::where("name", "like", "%" . $request->get("search") . "%")->select("id", "name", "MRP", "price", "basic_image", "selling_price", "discount", "product_color")->paginate(10);
         foreach ($product as $k) {
