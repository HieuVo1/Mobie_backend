...
class ProductController extends Controller {
     ...
     public function getattributedata(Request $request){
        ...
        $attributedrop=AttributeSet::whereHas('attributelist', function($q)use($id) {$q->where("is_delete",'0')->where("category",$id);})->where("is_deleted",'0')->get();
       foreach ($attributedrop as $k) {
     ...
     public function showaddcatalog(){
          $category=Categories::where("parent_category",0)->where("is_delete",'0')->get();
          $attributedrop=AttributeSet::with('attributelist')->whereHas('attributelist', function($q) {$q->where("is_deleted",'0');})->where("is_deleted",'0')->get();
     ...
     public function saveproductimage(Request $request){
        ...
        if($request->get("basic_img")!=""){
            if(strstr($request->get("basic_img"),"http")==""){
                $img = str_replace('data:image/png;base64,', '',$request->get("basic_img"));
                $img = str_replace(' ', '+', $img);
        ...
        if($request->get("additional_img")!=""){
             ...
             foreach (array_filter($data) as $k) {
                if(strstr($k,"http")==""){
                        $img = str_replace('data:image/png;base64,', '',$k);
                        $img = str_replace(' ', '+', $img);
                        $data = base64_decode($img);
                        $folderName = '/upload/product/';
                        $destinationPath = public_path() . $folderName;
